...
function tryFinish() {
	if (solved[0] && solved[1]) {
		...
		setTimeout(function () {
			...
			setTimeout(function () { $.post('https://mhacking/callback', JSON.stringify({ 'success': true, 'remainingtime': endTime - now })); }, 2000);
		}, 2000);
		resetGameState();
	}
}
...
function seedTable() {
	...
	if (now >= endTime) {
		...
		setTimeout(function () {
			...
			$.post('https://mhacking/callback', JSON.stringify({ 'success': false, 'remainingtime': 0 }));
		}, 2000);
		resetGameState();
	} else {
		writeTime();
