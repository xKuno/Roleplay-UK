$(document).ready(function(){
  ...
  $('input').change(function(){
    $.post('https://rpuk_clothes/updateSkin', JSON.stringify({
      value: false,
      // Face
      ...
      mask: $('.masks .active').attr('data'),
	  mask_texture: $('input[class=mask_2]').val(),
  ...
  $(document).keypress(function(e) {
    if(e.which == 97){ // A pressed
      $.post('https://rpuk_clothes/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 100){ // D pressed
      $.post('https://rpuk_clothes/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
	
    if(e.which == 120){ // X pressed
	  $.post('https://rpuk_clothes/closeall', JSON.stringify({
        value: 1
      }));	  
    }
  ...
  $('#tabs label').on('click', function(e){
    //e.preventDefault();
    $.post('https://rpuk_clothes/zoom', JSON.stringify({
