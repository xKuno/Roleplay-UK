...
Citizen.CreateThread(function()
	while true do
		...
		if not playing then
			for k, v in pairs(Config.ArcadeMachines) do
				local object = GetClosestObjectOfType(pedCoords, 5.0, v, false, false, false)
				if DoesEntityExist(object) then
					canSleep = 2000
					if #(pedCoords - GetEntityCoords(object)) < 1.5 then
						canSleep = 0
						local objCoords = GetEntityCoords(object)
						ESX.Game.Utils.DrawText3D(vector3(objCoords.x, objCoords.y, objCoords.z+1.0), "Press [~b~E~s~] to Play", 0.5, 6)
		...
		while true do
			...
			if showfailtext == false then
				...
				elseif IsControlPressed(2, 235) then
					...
					Citizen.Wait(150)
				elseif IsControlPressed(2, 177) then
					GameOver()
				else
					Citizen.Wait(0)
				end
