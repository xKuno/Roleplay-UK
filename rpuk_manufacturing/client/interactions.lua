...
function createInteraction(data)
	...
	self.secondaryAction = data.secondaryAction
	...
	self.secondaryInteractionKey = data.secondaryInteractionKey
	...
	while interactions[k] ~= nil and getDistance(coords, v.pos) < v.dist and not interactions[k].refreshInteractions do
	  ...
	  if getDistance(coords, v.pos) < v.dist and (v.canInteractWithCar or (not v.canInteractWithCar and (not (GetVehiclePedIsIn(ped, false) ~= 0) and (not exports.rpuk_health:deadStatus())))) then
		...
		if IsControlJustReleased(0, Keys[v.interactionKey]) and v.id == closestInteraction() and v.action ~= nil then
			v.action()
		elseif v.secondaryAction and v.secondaryInteractionKey and v.id == closestInteraction() then
			if IsControlJustReleased(0, Keys[v.secondaryInteractionKey]) then
				v.secondaryAction()
			end
		end
	  end
