...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			local armour = GetVehicleMod(vehicle, 16)
			if armour == -1 or armour == 0 then armour = 1
			else armour = armour + 1 end
			
			healthEngineCurrent = GetVehicleEngineHealth(vehicle)
			if healthEngineCurrent == 1000 then healthEngineLast = 1000.0 end
			healthEngineNew = healthEngineCurrent
			healthEngineDelta = healthEngineLast - healthEngineCurrent
			healthEngineDeltaScaled = healthEngineDelta * cfg.damageFactorEngine * (cfg.classDamageMultiplier[vehicleClass] / armour)

			healthBodyCurrent = GetVehicleBodyHealth(vehicle)
			if healthBodyCurrent == 1000 then healthBodyLast = 1000.0 end
			healthBodyNew = healthBodyCurrent
			healthBodyDelta = healthBodyLast - healthBodyCurrent
			healthBodyDeltaScaled = healthBodyDelta * cfg.damageFactorBody * (cfg.classDamageMultiplier[vehicleClass] / armour)
...
Citizen.CreateThread(function()
	while true do
		...
		local class = GetVehicleClass(vehicle)
		if class == 18 then -- Janky fix to stop clients being fucked over by door spam
			...
			SetVehicleAutoRepairDisabled(vehicle, true) -- doesn't work
		end
	end
end)